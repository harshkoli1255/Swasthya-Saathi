...
console.log("Adding ultra-modern soundwave screen...");
const addRes = execFileSync("node", ["scripts/flowstep_call.js", "add-screen", JSON.stringify({
  fileId: FILE_ID,
  label: "08 Voice Recording Listening (Active Waveform)",
  jsxContent: jsx
})], { encoding: "utf8" });

const screenData = JSON.parse(JSON.parse(addRes).result.content[0].text);
...
if (content.type === "image") {
  ...
  console.log(`✓ Saved: 08_voice_interview_recording_state.png (${buf.length} bytes)`);
}
